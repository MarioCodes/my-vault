    ...
    private void rellenoTablaDatos() {
        ...
        while(li.hasNext()) {
            try {
                ...
                model.addRow(row);
            }catch(NullPointerException ex) { //fixme: trampas, deberia comprobarlo y evitar los casos para que no salte nullPointerException. Pasa cuando hay algun elemento nulo en la vista (porque lo contiene alojamiento o actividad).
//                ex.printStackTrace();
            }
        }
    }
